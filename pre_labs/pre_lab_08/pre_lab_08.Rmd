...
**Task** Run the following code to add a month column to our dataframe using lubridate and count the number of calls for each month. In the space below, explain what each step of the code below is doing in English. Note the "label=TRUE" part in the mutate statement. **Step 1: adding a month column from the date column's designated month. Step 2: grouping the data frame by all spcific months, so likely all 12. Step 3: summarizing the data by making a total_calls column that counts the number of call per month. Step 4: Arranging the data set by the highst number of calls to the least number of calls.**
...
**Task** Run the following code. Explain what you see after running the code in the space below. **A blank square shows up.**
...
**Task** Run the following code. Explain what you see after running the code in the space below. **A bar chart where the number of bars corespond to the number of months and the height of the bars correspond to the total number of calls that month.**
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **The bars are ordered by the number of calls and not the month. The bars are arranged from least number of calls to the most number of calls.**
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **The x and y axes are flipped where the month is on the y axis and the count is on the x axis. It also shows the count descending from the most numbers of calls in a month to the month with the least number of calls.**
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **The background has changed from dark grey to white and the lines indicating the number label/number are now light grey.**
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last version of the chart? **The background is now a light blue, there are only lines corresponding to the Y axis (month) and the ticks of the count are on the inside of the graph. (its ugly)**
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **A title is added and labels for each axis are modified. **
...
**Task** Run the following code. Explain what you see after running the code in the space below. **The total number of cllas by the individual date.**
...
**Task** Run the following code. Explain what you see after running the code in the space below. **A line chart of the number of calls by the spcific date.**
...
**Task** Run the following code. Explain what you see after running the code in the space below. **The same thing just that the lables on the x axis are now every single week, overlapping in the graph.**
...
**Task** Run the following code. Explain what you see after running the code in the space below. **The week labels tilted for better readbility.**
