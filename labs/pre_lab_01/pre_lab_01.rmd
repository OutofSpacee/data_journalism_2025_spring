...
The data type for the "id" column is chr and for the "seats" column is dbl
...
```{r}
umd_courses |> # pipe operator takes the umd_courses var to do things to it
  group_by(term) |>
  summarise(
    count_classes = n()
  )
...
umd_courses |> Selects the data set
  group_by(term) |> Groups the data by the column "term"
  summarise(    ... allows operations on the group_by 
    count_classes = n() counting and stores the # of classes
  )
...
umd_courses |>
  ...
  arrange(desc(count_classes)) # Allows for the descending order viewing
...
```{r}
umd_courses |>
  group_by(term, department) |> # Grouping by two columns
                                # Bucket for every combo
  summarise(
    count_classes = n()
  ) |>
  arrange(term, department)     # which comes first
```
...
umd_courses |>
  ...
  summarise(
    count_classes = n(),
    total_seats = sum(seats) # Taking the total of all seats in a term
  ) |>
...
umd_courses |>
  ...
  summarise(
    ...
    mean_seats = mean(seats), # getting the mean
    median_seats = median(seats) # getting the median 
  ) |>
  arrange(desc(total_seats))
```
...
umd_courses |>
  ...
  summarise(
    ...
    min_seats = min(seats),   # Mix func
    max_seats = max(seats)   # Max func
  ) |>
  arrange(min_seats)
```
...
1.  Which department has the highest number of possible credits by term

2.  What is the average number of classes an instructor teaches

3.  Which department has had the most growth in seat # and which department has
    had a decrease in the # of seats
...
selected_jour_courses <- journalism_courses |> 
  select(id,title) #selects certain columns

head(selected_jour_courses)
```
